...
def update(person_id, note_id, note):
    ...
    if update_note is not None:
        ...
        schema = NoteSchema()
        update_load = schema.load(note, session=db.session)

        # Set the id's to the note we want to update
        update_load.person_id = update_note.person_id
        update_load.note_id = update_note.note_id

        # merge the new object into the old and commit it to the db
        db.session.merge(update_load)
        db.session.commit()
